...
/******************************************************************
************************ PROCESSOS/THREADS ***********************/
char* active_threads[MAX_THREADS];
...
int is_active_thread(char *name) {
    for (int i = 0; i < MAX_THREADS; i++) {
        if (!strcmp(active_threads[i], name)) return 1;
    }
    return 0;
}
...
void fcfs(FILE *file, Process **processes, int n) {
    ...
    while (i < n || !is_queue_empty(q) || running > 0) {
        ...
        if (running < MAX_THREADS && !is_queue_empty(q)) {
            pthread_t thread;
            pthread_mutex_lock(&lock);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < MAX_THREADS; i++) active_threads[i] = NULL;
